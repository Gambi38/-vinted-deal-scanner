#!/usr/bin/env python3
# VERSION : SCANNER_VINTED_FR_V6_5_FILTRES_PERSONNELS
import asyncio
import csv
import json
import os
...
ANCIEN_CONFIG_PATH = ROOT / "config.json"
BLACKLIST_PATH = ROOT / "liste_noire.json"
ANCIEN_BLACKLIST_PATH = ROOT / "blacklist.json"
...
def convertir_filtre_personnel(entree):
    """Transforme un filtre simple en règle interne du scanner."""
    if not isinstance(entree, dict) or not entree.get("actif", True):
        return None

    nom = str(entree.get("nom", "")).strip()
    recherche = str(entree.get("recherche_vinted", "")).strip()
    categorie = str(entree.get("categorie", "")).strip()

    if not nom or not recherche or not categorie:
        return None
...
def appliquer_filtres_personnels(cfg, blacklist):
    ...
    for entree in donnees.get("articles_a_surveille", []):
        recherche = convertir_filtre_personnel(entree)
        if recherche:
            recherches_perso.append(recherche)
...
async def main():
    ...
    if not cfg:
        print(
            "configuration.json introuvable."
        )
        sys.exit(1)

    if not blacklist:
        print(
            "ATTENTION: liste_noire.json "
            "introuvable ou vide."
        )
