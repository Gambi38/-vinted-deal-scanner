#!/usr/bin/env python3
# VERSION : VINTED_V74_PRIX_REALISTES_RETRO
#
# Surcouche V6.9 :
    ...
    for search in cfg.get("searches", []):
        ...
        for rule in search.get("rules", []):
            ...
            if categorie == "JEU_RETRO":
                ...
                try:
                    rule["resale_low"] = min(float(bas), 45.0) if bas is not None else 45.0
                except Exception:
                    rule["resale_low"] = 45.0
                try:
                    rule["resale_high"] = min(float(haut), 65.0) if haut is not None else 65.0
                except Exception:
                    rule["resale_high"] = 65.0
...
if __name__ == "__main__":
    print("Vinted V7.4 — PRIX REALISTES + RETRO STRICT + vision + rejets appris")
    try:
        asyncio.run(vt.main())
