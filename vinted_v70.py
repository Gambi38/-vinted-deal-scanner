#!/usr/bin/env python3
# VERSION : VINTED_V712_RECUPERATION_TRIAGE
#
# Surcouche V6.9 :
...
# ---------------------------------------------------------------------------
# V7.12 - RECUPERATION DES VRAIS PRODUITS AU CONTROLE PROFOND
# ---------------------------------------------------------------------------
# Vinted peut afficher "PS5" / "Switch" dans la carte du catalogue puis fournir
# un og:title très court sur la page détaillée ("Elden Ring Nightreign", etc.).
# L'ancien contrôle deep exigeait alors de retrouver la plateforme et rejetait
# silencieusement un vrai jeu. On autorise un secours UNIQUEMENT quand :
# - le modèle du jeu correspond fortement au titre détaillé ;
# - aucune plateforme incompatible n'est explicitement présente ;
# - il s'agit d'une règle jeu avec une plateforme identifiable.
# Les blacklists, emballages, accessoires et plateformes contradictoires restent
# contrôlés avant et après ce secours.

_rule_match_v69_orig_v712 = rule_match_v69

def _plateforme_attendue_v712(rule):
    termes = [n(x) for x in rule.get("platform_any", []) if str(x).strip()]
    appris = n(str(rule.get("_plateforme_apprise", "") or ""))
    blob = " ".join(termes + ([appris] if appris else []))
    if "playstation 5" in blob or "ps5" in blob:
        return "ps5"
    if "playstation 4" in blob or "ps4" in blob:
        return "ps4"
    if "switch" in blob:
        return "switch"
    if "3ds" in blob:
        return "3ds"
    if re.search(r"\bds\b", blob):
        return "ds"
    if "vita" in blob:
        return "vita"
    if "psp" in blob:
...
def _mots_identite_v712(rule):
    source = str(rule.get("model") or rule.get("label") or "")
    inutiles = {
        "nintendo","playstation","sony","microsoft","switch","ps5","ps4",
        "ps3","ps2","jeu","game","edition","version","the","of","and",
        "for","pour","avec","remastered","remaster"
    }
    mots = [
        x for x in re.findall(r"[a-z0-9]{2,}", n(source))
        if x not in inutiles
    ]
    return list(dict.fromkeys(mots))[:8]

def rule_match_v712(rule, title, text, deep=False):
    ok = _rule_match_v69_orig_v712(rule, title, text, deep=deep)
    if ok or not deep:
        return ok

    attendu = _plateforme_attendue_v712(rule)
    if not attendu:
        return False

    # Seulement les règles de jeux : jamais pour une console ou un accessoire.
    kind = n(str(rule.get("_type_produit", "") or ""))
    if kind and kind != "jeu":
        return False

    full = n(f"{title} {text}")
    mauvais = PLATEFORMES_INCOMPATIBLES.get(attendu, [])
    bon = PLATEFORMES.get(attendu, [])

    mauvais_hits = hits(full, mauvais)
    bon_hits = hits(full, bon)

    # Une plateforme explicitement contradictoire reste un rejet dur.
    if mauvais_hits and not bon_hits:
        return False

    mots = _mots_identite_v712(rule)
    if not mots:
        return False

    presents = [m for m in mots if present(full, m)]
    # 1 mot seulement n'est accepté que pour un modèle lui-même très court.
    minimum = 1 if len(mots) == 1 else max(2, int((len(mots) + 1) * 0.55))
    if len(presents) < minimum:
        return False

    # Ne sauve jamais les dérivés/accessoires ayant repris le nom du jeu.
    if hits(title, DERIVES_JEU) or accessoire_jeu_titre_v710(title):
        return False

    print(
        f"  ↩ RECUPERATION TRIAGE | {str(title)[:58]} | "
        f"modèle reconnu, plateforme détaillée absente"
    )
    return True

# Les appels directs internes et le moteur V6.8 doivent utiliser la même logique.
rule_match_v69 = rule_match_v712
...
if __name__ == "__main__":
    print("Vinted V7.12 — RECUPERATION TRIAGE + FLASH OUVERT + BANGER EXPRESS")
    try:
        asyncio.run(vt.main())
